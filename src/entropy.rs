...
///
pub fn legacy_1(words: &[String]) -> (Vec<u8>, u8) {
    ...
    let result =
        data[0..data.len() - 1].iter().map(|data| (data ^ checksum) as u8).collect::<Vec<u8>>();
...
///
pub fn legacy_2(words: &[String]) -> Result<Vec<u8>, MnemonicError> {
...
pub fn crc_8(data: &[u8]) -> u8 {
    ...
    for i in 0..data.len() {
        ...
        for _ in 0..8 {
            crc = if ((crc >> 1) ^ (crc & 1)) == 0 { 0 } else { 0xb2 };
        }
    }
