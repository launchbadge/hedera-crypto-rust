use ed25519_dalek::{ExpandedSecretKey, Keypair, PublicKey, SecretKey, Signature, Signer, SECRET_KEY_LENGTH, SIGNATURE_LENGTH, KEYPAIR_LENGTH};
...
pub struct PrivateKey{
    keypair: Keypair,
    chain_code: Option<Box<[u8; 32]>>
}
impl PrivateKey {
    pub fn generate() -> PrivateKey {
        let mut csprng = OsRng{};
        let keypair: Keypair = Keypair::generate(&mut csprng);
        PrivateKey{keypair, chain_code: None}
    }
        ...
        let private_key = match data.len() {
            32 => {
                let private_key = PrivateKey {
                    keypair: Keypair::from_bytes(&data).map_err(KeyError::Signature)?,
                    chain_code: None,
                };
                private_key
            }
            48 if data.starts_with(&der_prefix_bytes) => {
                let private_key = PrivateKey {
                    keypair: Keypair::from_bytes(&data[der_prefix_bytes.len()..])
            ...
            64 => {
                let private_key =
                    PrivateKey{keypair: Keypair::from_bytes(&data[..KEYPAIR_LENGTH])?, chain_code: None};
                private_key
            }
            _ => {
...
mod tests {
    ...
    const PRIVATE_KEY_BYTES: &[u8; SECRET_KEY_LENGTH] = &[
        ...
        -116, -50, 122, -15, -69, -113, 57, -55, 119, 50, 57, 68, -126, 83, -114, 16
    ];
    #[test]
    fn test_generate() -> Result<(), KeyError> {
        let private_key = PrivateKey::generate();
        assert_eq!(private_key.to_bytes().len(), 32 as usize);
        Ok(())
    }
    #[test]
